...
function NavItem({ to, activeTab, setActiveTab, label, children }: NavItemTypes) {
    ...
    return (
        ...
        <li className={`relative  top-0  p-1 rounded-full w-auto transition-all duration-300 ${isActive ? "bg-[#009CEA]" : "bg-black group-hover:bg-[#009CEA] "  }  `} onClick={() => setActiveTab(to)}>
            <Link to={to} className={` "flex flex items-center justify-start transition-all" ${isActive ? " duration-300 w-fit" : "group-hover:w-fit" } `}>
                <svg xmlns="http://www.w3.org/2000/svg" fill="#f8f8f8" viewBox="0 0 30 30" strokeWidth="0" stroke="none"
                    className={` size-10 transition-all duration-300  translate-x-[3px] translate-y-[2px]
                    ${isActive ? "stroke-[#f8f8f8] " : "group-hover:stroke-[#f8f8f8] "}`}>
                    {children}
                </svg>

                {/* Texte placé exactement comme le hover */}
                {isActive && (
                <span className={`text-[#f8f8f8] text-sm font-medium transition-all top-3 ml-1 mr-2 left-11 duration-300 
